 ...
 */
export const useRenderBlocks = (blocks: BlockData[] | undefined,
  isEditable: boolean,
  editorCallbacks?: EditorCallbacks,
  currentNestingLevel = 0,
  draggedBlockType: string | null = null,
  isChildAllowedInParent?: (childType: string, parentType: string) => boolean): VNode[] => {
  if (!blocks) {
    return [
    ]
  }
  ...
  return blocks.
    ...
    map((block) => {
      ...
      const specificBlockComponentProps: GenericBlockComponentProps = {
        ...
        renderBlocks: (childBlocks, isEdit, callbacks, level) => useRenderBlocks(childBlocks, isEdit, callbacks, level, draggedBlockType, isChildAllowedInParent),
        editorCallbacks: editorCallbacks,
      ...
      const renderedSlots = blockSchemaEntry.nestable && `slots` in block && block.slots ? Object.entries(block.slots).map(([
        slotName,
        slotBlocks
      ]) => h(BlockSlot, {
