 ...
 */
export const createCalloutBlock = (initialData?: Partial<CalloutBlockData>): CalloutBlockData => {
  return createBaseBlock<CalloutBlockData>(`callout`, {
    attrs: {
      variant: initialData?.attrs?.variant || `note`,
      text: Array.isArray(initialData?.attrs?.text) ? initialData?.attrs?.text : ([
        {
          type: `text`,
          text: String(initialData?.attrs?.text || ``)
        }
      ] as TextNode[])
    }
  })
